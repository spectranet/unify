...
echo "<hr><h1>API getUpdatedAccountList:</h1>"; 
$getvol = getUpdatedAccountList($from_date,$to_date);
#print_r($getvol); 
$xml = new array2xml($getvol, "getUpdatedAccountList");
print_r($xml->output);
 

function getUpdatedAccountList($from_date,$to_date){ 
$params = array(  new XML_RPC_Value($from_date, 'dateTime.iso8601'),
                  new XML_RPC_Value($to_date, 'dateTime.iso8601')  # date Bill end date
          );
$msg1 = new XML_RPC_Message('unify.getUpdatedAccountList', $params); 
...
$resp1 = $cli1->send($msg1); 
 ####################  Logging #############
 $FTTH_LOG = "\n######################################\n<br>";
 $FTTH_LOG .= "unify.getUpdatedAccountList request param:\n" . $msg1->serialize ()."<br>";
 $FTTH_LOG .= "\n######################################\n<br>";
 $FTTH_LOG .= "unify.getUpdatedAccountList response:\n" . $resp1->serialize ()."<br>";
# echo $FTTH_LOG;
        if (!$resp1)  
        { 
